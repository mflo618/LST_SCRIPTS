...
These scripts are real computations, not mocks. They make no network calls and use standard-library arithmetic. Decimal arithmetic is used for the closure calculations; the single non-integer power `phi^(4*phi)` in `derive_scale_alpha_relational.py` uses Python's standard `math.exp/log` path to avoid Decimal transcendental slowdowns while preserving the displayed 10-12 digit audit precision.
